...
internal sealed class SuperBusTransport : ITransport, IDisposable
{
    ...
    )
    {
        ...
        _log = loggerFactory.GetLogger<SuperBusTransport>();
        _messageLockRenewalTask = asyncTaskFactory.Create(
            "SuperBus Rebus transport message lock renewal",
            RenewPeekLocks,
    ...
    {
        // Intentionally not implemented. The SuperBus infrastructure is responsible
    ...
    {
        if (Address is null)
            throw new InvalidOperationException("This SuperBus transport does not have an input queue, which means that it is configured to be a one-way client. Therefore, it is not possible to receive anything.");
    ...
    public Task Send(string destinationAddress, TransportMessage transportMessage, ITransactionContext context)
    {
        ...
        {
            var messagesToSend = new ConcurrentQueue<(string Destination, SuperBusMessage Message)>();
