...
using static SuperBus.Client.Core.AmqpAnnotatedMessageConverter;
...
namespace SuperBus.Client.Core
{
    internal class AmqpMessageConverter
    {
        ...
        public virtual AmqpMessage BatchSBMessagesAsAmqpMessage(SuperBusMessage source, bool forceBatch = false)
        {
        ...
        public virtual AmqpMessage BatchSBMessagesAsAmqpMessage(IReadOnlyCollection<SuperBusMessage> source, bool forceBatch = false)
        {
            //Argument.AssertNotNull(source, nameof(source));
        ...
        ///
        private AmqpMessage BuildAmqpBatchFromMessage(IReadOnlyCollection<SuperBusMessage> source, bool forceBatch)
        {
            var batchMessages = new List<AmqpMessage>(source.Count);
            foreach (SuperBusMessage sbMessage in source)
            {
        ...
        public virtual AmqpMessage SBMessageToAmqpMessage(SuperBusMessage sbMessage)
        {
