...
namespace Dbosoft.Bote.Client.Core
{
    ...
    /// </summary>
    ///
    internal class AmqpMessageBatch
    {
        ...
        /// </summary>
        ///
        public int Count => _batchMessages.Count;
        ...
        ///
        public IReadOnlyCollection<T> AsReadOnly<T>()
        {
            if (typeof(T) != typeof(AmqpMessage))
            {
                throw new FormatException(string.Format(CultureInfo.CurrentCulture, ResourcesTodo.UnsupportedTransportEventType, typeof(T).Name));
            }
