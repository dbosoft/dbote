...
/// <summary>
///   A set of <see cref="BoteMessage" /> with size constraints known up-front,
///   intended to be sent to the Queue/Topic as a single batch.
///   A <see cref="BoteMessageBatch"/> can be created using
///   <see cref="BoteSender.CreateMessageBatchAsync(System.Threading.CancellationToken)"/>.
///   Messages can be added to the batch using the <see cref="TryAddMessage"/> method on the batch.
/// </summary>
///
public sealed class BoteMessageBatch : IDisposable
{
    /// <summary>
    ...
    /// <summary>
    ///   Initializes a new instance of the <see cref="BoteMessageBatch"/> class.
    /// </summary>
    ///
    ...
    /// </remarks>
    ///
    internal BoteMessageBatch(ServiceBusMessageBatch servicebusBatch)
    {
    ...
    ///   Occurs when the <paramref name="message"/> has a member in its <see cref="BoteMessage.ApplicationProperties"/> collection that is an
    ///   unsupported type for serialization.  See the <see cref="BoteMessage.ApplicationProperties"/> remarks for details.
    /// </exception>
    ///
    //public bool TryAddMessage(BoteMessage message)
    //{
    //   return _innerBatch.TryAddMessage(message.GetServiceBusMessage());
    //}

    /// <summary>
    ///   Performs the task needed to clean up resources used by the <see cref="BoteMessageBatch" />.
    /// </summary>
    ///
    public void Dispose()
    {
