...
[StepDocumentation("Enforces Bote connector ID header")]
internal class BoteOutgoingConnectorStep(string connectorsQueue) : IOutgoingStep
{
    public async Task Process(OutgoingStepContext context, Func<Task> next)
    {
        ...
        {
            if (outgoingMessage.Headers.TryGetValue(BoteHeaders.ConnectorId, out var headerConnectorId))
            {
            ...
            {
                outgoingMessage.Headers[BoteHeaders.ConnectorId] = connectorId;
            }
        }
